...
# assignment: Lab 6
    ...
    stack_beg:     .word   0 : 40
    stack_end:     #.align 2 
    input1:        .word 0:4                                   # holds first user input after ascii to int conversion
    input2:        .word 0:4                                   # holds second user input after ascci to int conversion
    result:        .word 0:4                                   # holds computed value or quotient
    remainder:     .word 0:4                                   # holds remainder
    tempVar:       .word 0:4                                   # holds temp quotient for modulus 
    operand:       .word 0:4                                   # holds operand
    buffer:        .byte 0 : 80                                # temp holds user ascii numeric input
    prompt1:       .asciiz "Enter 1st Number: "                # prompt for first user input
    prompt2:       .asciiz "Enter operand or 'q' to exit: : "  # prompt for operand
    prompt3:       .asciiz "Enter 2nd Number: "                # prompt for second user input
...
_firstInput:
    ...
    la  $a0, prompt1        # load address of input prompt1 string to print
    la  $a1, input1         # load address of input1 for first user input
    jal GetInput            # goto function that gets user numeric input
_operatorInput:
    la  $a0, prompt2        # load address of operand prompt string to print
    jal GetOperator         # goto function that gets operator
    sw  $v1, operand        # operand = $v1, store operand
    
    beq $v1, '$', _sqrtOp   # if $v1 == squareroot operand then skip second input
    beq $v1, 'q', ExitPrgm  # if $v1 == 'q' then exit program
    
_secondInput:
    la  $a0, prompt3        # load address of input prompt3 string to print
    ...
    la  $a3, remainder      # load address of remainder
    
    la $ra, _displaySetup   # load address to where branching statements return to after execution
    beq $v1, '+', AddNumb   # if $v1 == '+' then perform $a0 + $a1
    beq $v1, '-', SubNumb   # if $v1 == '-' then perform $a0 - $a1
    beq $v1, '*', MultNumb  # if $v1 == '*' then perform $a0 * $a1
    beq $v1, '/', DivNumb   # if $v1 == '/' then perform $a0 / $a1
    beq $v1, '%', ModNumb   # if $v1 == '$' then perform $a0 mod $a1
_sqrtOp:
    la  $a0, input1         # load address of 1st input
    jal SquareRoot          # goto function to calculate square root

    la  $a0, input1         # load address where calculated result resides
    la  $a1, result         # load address of result
    jal VarCpy              # goto function to copy contents of $a0 to $a1
_displaySetup:
    la  $a0, resulthead     # load address of result dialog
    la  $a1, result         # load address of result, i.e., $a1 = result
    jal DisplayNumb         # goto function to display result of calculation

    bne $v1, '/', MainLoop  # if $v1 != '/' then restart MainLoop else display remainder

    la  $a0, remainderhead  # load address of remainder string dialogue
    la  $a1, remainder      # load address of remainder  
    jal DisplayNumb         # goto function to display remainder of division
    j MainLoop
ExitPrgm:
    li  $v0, 10
    syscall
...
GetInput:
    ...
    sw   $a1, 0($sp)   # push $a1 to stack in order to save original value
    sw   $ra, 4($sp)   # push $ra to stack so that jump back to main is saved
    
    la   $t0, buffer   # load address of buffer
    li   $v0, 8        # specify String read service
    addi $a0, $t0, 0   # load address of input buffer
    li   $a1, 80       # max number of characters to read, i.e., $a1 = max terminal length
    syscall            # read user input
    
    lw   $a1, 0($sp)   # restore $a1 to original value, i.e., $a1 = input variable address   
    jal DecAscToBin    # goto DecAscToBin for ascii to binary conversion function
    lw   $ra, 4($sp)   # restore $ra for jump back to main
...
DisplayNumb:
    ...
    addi $sp, $sp -4       # save space for 1 word, decrement the stack pointer 
    sw $ra, 0($sp)         # push $ra to stack so that jump back to main is saved
    
    addi $a0, $a1, 0       # $a0 = calculation result
    la   $a1, buffer       # load address of buffer
    jal  BinToDecAsc       # goto function to convert from binary back to ascii
    
    li $v0, 4              # specify Print String service
    la $a0, buffer         # load address of calculation result buffer
    syscall                # print calculation

    lw $ra, 0($sp)         # restore $ra for jump back to main
    addi $sp, $sp, 4       # increment stack to original position
    jr $ra                 # goto main loop

####################################################################################
# Procedure: ModNumb  $a0 mod $a1 = $a2
# Calculates modulus 
# Input: $a0 points to a word address in .data memory, where the dividend is stored
# Input: $a1 points to a word address in .data memory, where the divisor is stored
# Output: $a2 points to a word address in .data memory, where to store the result
ModNumb:
    addi $sp, $sp -4        # save space for 1 word, decrement the stack pointer 
    sw $ra, 0($sp)          # push $ra to stack so that jump back to main is saved
    
    move $a3, $a2           # $a3 = $a2, i.e., remainder in DivNumb is result for modulus
    la $a2, tempVar         # $a2 = holds quotient of DivNumb, not needed for modulus 
    jal DivNumb             # goto function that provides quotient and remainder
